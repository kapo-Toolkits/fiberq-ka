Thank you for the translation guide — `docs/TRANSLATING.md` and the `<extracomment>` notes in the catalogue are the most considerate translator documentation I have come across in an open-source project, and several decisions below could not have been made without them.

## Language

**Georgian (ka) — ქართული.**

Single script (Mkhedruli), no regional variants: `ka_GE` is the only locale in practical use, so one catalogue covers everything. Georgian has one plural form in Qt/CLDR, so every `%n` message needs a single `<numerusform>`.

## What I would like to do

**Start a brand-new translation for this language.** Per CONTRIBUTING.md I am not creating `fiberq/i18n/fiberq_ka.ts` myself — please generate it when convenient.

## How familiar I am with fibre terminology

**Not familiar with the terminology, but fluent in the target language.**

I want to be straightforward about this, because it changes how the glossary below should be read. My background is GIS and QGIS plugin development, not fibre plant. The Georgian terms are derived from three things: your own `<extracomment>` notes, which define each concept precisely enough to translate; established Georgian engineering and utility vocabulary (`ტრასა` for an alignment, `მუფტა` for a joint closure, `ბოძი` for a pole, `მალი` for a span); and the existing QGIS Georgian UI translation for the GIS half.

That is enough to produce something accurate and internally consistent, and I think it is. It is **not** the same as a Georgian fibre engineer signing off on the vocabulary. If you know of anyone in a Georgian ISP or a contractor who could look over the glossary, I would very much welcome it — a terminology review is worth more here than a second translator.

## How I would prefer to contribute the file

**Git clone and pull request.** Qt Linguist is already available here via QGIS (`apps/Qt5/bin/linguist.exe` in QGIS 3.44, `apps/qt6/bin/linguist.exe` in QGIS 4.2), so editing is Route A and delivery is a PR. I will send the `.ts` only and leave the `.qm` build to you.

## Terminology notes

I prepared a full English → Georgian glossary before starting, in the same shape as the French and Serbian ones in CONTRIBUTING.md — about 130 terms across structures and civil works, cables, splicing and slack, elements, network architecture, buildings, CAD drawings, selection, GIS vocabulary and the validation engine. It is at the bottom of this issue. Happy to have it merged into CONTRIBUTING.md as the Georgian section in whatever form suits you.

Three of your translator notes changed what I would otherwise have written, and are worth calling out because they are exactly the cases the notes exist for:

1. **`Object` = building.** Georgian QGIS already translates the GIS term *feature* as `ობიექტი` — literally "object". Without your note I would certainly have used that word for FiberQ's `Object`, and been wrong in about ten strings. The Georgian catalogue uses `შენობა` (building) for `Object` and keeps `ობიექტი` for `Feature`. Counter-intuitive, but correct.

2. **`Route` is the physical alignment on the ground.** Georgian has a dedicated engineering term for this, `ტრასა`, distinct from `მარშრუტი` (a travel route), which is what any general translator would reach for.

3. **`Drop` as a noun, and `Add breakpoint` as a geometry split rather than a fibre fault.** Both are handled with terms that cannot be misread as the verb, or as a cable fault respectively — `Fiber break` is a separate string and stays lexically separate in Georgian.

One string where the English is genuinely ambiguous and I would rather ask than guess, as TRANSLATING.md advises:

- **`List of latent elements`** — what does "latent" mean here? Unplaced, pending approval, hidden from the map, or something else? I have left it untranslated in the glossary until you can confirm.

Two smaller points, no answer needed unless you disagree:

- The quick-toolbar buttons `Aerial Cable` and `Underground Cable` are fixed to the backbone subtype in code but the label does not say so. I plan to mirror the English's silence rather than make the Georgian explicit — say if you would prefer otherwise.
- `Object` (singular) and `Objects` (plural) title sibling message boxes for the same feature. Your note already flags this as an inconsistency in the English; both render as the same concept in Georgian.

## Progress and timescale

I have worked ahead of the catalogue so that typing it in is mechanical once the file exists. Translated so far, against the current `fiberq_fr.ts` as the source of English strings:

| Group | Strings | Status |
|---|---:|---|
| `FiberQ`, `ElementNames`, `CableLayingUI`, `RoutingUI`, `ObjectsUI`, `DrawingsUI`, `SlackUI`, `DuctingUI`, `SelectionUI`, `ElementPlacementUI`, `QuickToolbar` | 76 | done |
| `ValidationRules` | 45 | done |
| `ValidationReport` | 28 | to do |
| `ValidationPanel` | 21 | to do |
| `FiberQPlugin` | 123 | to do |
| **Total** | **293 messages / 306 slots** | **121 (~40%)** |

I will send in batches rather than hold a finished file.

## Credit

**Giorgi Kapanadze (@ezdanapak)**, please.

## Three documentation points noticed while preparing

These are documentation rather than translation issues. Happy to open them separately or send a PR — whichever you prefer.

1. **`docs/TRANSLATING.md` lists ten placeholders.** The catalogue currently uses 34 distinct ones; the validation rules added `{tol}`, `{crs}`, `{fid}`, `{expected}`, `{computed}`, `{bound}`, `{allowed}`, `{layers}`, `{rules}`, `{features}`, `{summary}` and others.

2. **The context table in `docs/TRANSLATING.md` predates v1.4.0.** It omits `ElementNames` (12), `ValidationPanel` (21), `ValidationReport` (28) and `ValidationRules` (45) — 106 strings, more than a third of the catalogue — and gives `FiberQPlugin` as 105 where it is now 123. A translator budgeting their time from that table would be planning for roughly two thirds of the real work, which seems worth fixing given how carefully the rest of the guide manages expectations.

3. **`fiberq/i18n/__init__.py` `_LANGUAGE_NAMES` has no `ka` entry**, so Georgian would appear in the language menu as the bare code `ka`. The docstring says unknown codes fall back to the code deliberately, so nothing breaks — it is one line to make it read properly:
...
- [x] **Human translation.** This is human translation. No machine translation engine was used, including for the glossary.
- [x] **Licence.** I understand my contribution will be released under GPL-3.0-or-later, the same licence as the rest of FiberQ.
- [x] **Rule 1.** I will never edit the English `<source>` text — only the `<translation>` slot.
- [x] **Rule 2.** I will keep placeholders exactly as written, moving them within the sentence only where Georgian syntax requires it.
- [x] **Rule 3.** I will not compile anything. I will submit only the `.ts` file and leave the `.qm` build to you.

---

<details>
<summary><b>English → Georgian glossary (draft v0.3) — click to expand</b></summary>

## English → Georgian glossary (draft)

For the FiberQ Georgian catalogue `fiberq/i18n/fiberq_ka.ts`.

Status: **draft, v0.3** — proposed by the Georgian translator, not yet used in the
...
a practitioner's judgement. Terms marked
**(ask maintainer)** are ones where the *English* is ambiguous and the meaning
must be settled before any term is chosen.
...
| Relations | **კავშირები** | |
| Latent elements | **(ask maintainer)** | `List of latent elements` — the English is unclear. "Latent" could mean unplaced, pending, or hidden. No Georgian term proposed until the meaning is confirmed. |
...
1. **`List of latent elements`** — what does "latent" mean here? Unplaced, pending
   approval, hidden from the map, or something else? No Georgian term can be chosen
   until this is settled. (This is the "flag it rather than guess" case that
   `TRANSLATING.md` asks for.)
