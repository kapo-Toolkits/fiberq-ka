...
The terminology reference behind the Georgian translation of FiberQ — about 140 terms
across structures, cables, splicing and slack, elements, network architecture, buildings,
CAD drawings, selection, GIS vocabulary and the validation engine.

!!! abstract "Provenance"
    Structured to mirror the English → French and English → Serbian glossaries in
    FiberQ's own `CONTRIBUTING.md`, and offered upstream in
    [issue #43](https://github.com/vukovicvl/fiberq/issues/43).

    Definitions come from the maintainer's `<extracomment>` notes in the catalogue, from
    established Georgian engineering and utility vocabulary, and from the existing QGIS
    Georgian UI translation. The author is **not** a fibre practitioner — a terminology
    review by one would be welcome.

!!! info "Canonical copy"
    The version submitted upstream lives at
    [`GLOSSARY-ka.md`](https://github.com/kapo-Toolkits/fiberq-ka/blob/main/GLOSSARY-ka.md)
    in the repository root. This page is generated from it, so edit that file rather than
    this one.
